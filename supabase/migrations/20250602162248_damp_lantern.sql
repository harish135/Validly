/*
  ...
  1. New Tables
    - subscription_plans: Defines available plans and their limits
  ...
  3. Functions
    - get_user_plan_and_usage: Tracks remaining usage
*/
...
DO $$ BEGIN
    ...
    IF NOT EXISTS (
        ...
        CREATE POLICY "System can insert feature usage (via trusted functions)" ON public.feature_usage_tracking
            FOR INSERT TO public WITH CHECK ((auth.uid() = user_id) OR is_admin());
    END IF;
