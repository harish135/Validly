...
BEGIN
    RETURN QUERY
    WITH daily_usage AS (
        SELECT COALESCE(SUM(usage_count)::integer, 0) as total_minutes
    ...
    SELECT 
        ...
        COALESCE(du.total_minutes, 0)::integer as minutes_used,
        CASE 
            WHEN sp.daily_time_allowance_minutes IS NULL THEN -1
            ELSE GREATEST(0, sp.daily_time_allowance_minutes - COALESCE(du.total_minutes, 0))::integer
