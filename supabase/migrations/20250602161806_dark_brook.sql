/*
  ...
  1. New Tables
    - subscription_plans: Defines available plans and their time limits
    - user_subscriptions: Tracks user's current subscription
    - feature_usage_tracking: Logs feature usage
    - feature_usage: Aggregates usage statistics
  
  2. Security
    - RLS enabled on all tables
    - Policies for public viewing and admin management
    
  3. Functions
    - get_user_plan_and_usage: Calculates remaining usage time
*/
...
DO $$ BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'feature_usage_tracking' 
        AND policyname = 'Users can insert their own feature usage (via trusted functions'
    ) THEN
        CREATE POLICY "Users can insert their own feature usage (via trusted functions" ON public.feature_usage_tracking
