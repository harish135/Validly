...
import type { MyReportItem, ReportData } from '../types'; // Renamed HistoryItem
import { CONFIDENCE_TEXT_COLORS } from '../constants';
import IconButton from './IconButton';
import CheckCircleIcon from './icons/CheckCircleIcon';
import ShareIcon from './icons/ShareIcon'; // New Icon

interface MyReportsModalProps { // Renamed from HistoryModalProps
  isOpen: boolean;
  onClose: () => void;
  myReportItems: MyReportItem[]; // Renamed from historyItems
  onSelectMyReportItem: (reportData: ReportData) => void; // Renamed
}
...
const MyReportsModal: React.FC<MyReportsModalProps> = ({ isOpen, onClose, myReportItems, onSelectMyReportItem }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="My Reports (Current Session)" size="xl"> {/* Wider modal for Share button */}
      {myReportItems.length === 0 ? (
        ...
        <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
          {myReportItems.map((item) => (
            <div 
              key={item.id} 
              className="p-3 bg-brand-gray-700 rounded-md border border-brand-gray-600 hover:border-brand-premium-blue transition-colors group"
            >
              <div className="flex justify-between items-center">
                <div className="flex-grow mr-2 min-w-0"> {/* Added min-w-0 for flex child truncation */}
                ...
                <div className="flex items-center flex-shrink-0 space-x-2">
                  <div data-tooltip="Share with team - Available on Pro Plan">
                    <IconButton
                      ...
                      onClick={(e) => e.stopPropagation()} // Prevent row click while allowing tooltip
                      variant="ghost"
                      size="sm"
